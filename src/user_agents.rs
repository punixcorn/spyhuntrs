...
pub const user_agent_file: &str = "user_agents.txt";

/// fetches user agents from useragentsstring.com
/// if it could not fetch , dont kill program or stop flow, we can use the array of user agents
pub async fn fetch_useragents_from_internet() -> Result<Vec<String>, Box<dyn Error>> {
    let client = reqwest::Client::builder().build()?;
    let response = client
        .get("https://www.useragentstring.com/pages/Browserlist/")
        .send()
        .await?;
    let mut user_agents: Vec<String> = vec![];

    if response.status() != StatusCode::OK {
        warn!("Could not fetch useragent data");
    } else {
        let response_data = response.text().await?;

        let document = scraper::Html::parse_document(&response_data);
        let agent_selector = scraper::Selector::parse("li > a").unwrap();

        let agent = document
            .select(&agent_selector)
            .map(|x| println!("{}", x.inner_html()));

        // Loop through each <a> tag, extract the text (user agent)
        user_agents = document
            .select(&agent_selector)
            .filter_map(|a| {
...
pub fn fetch_pre_existing_user_agent() -> String {
    let num = rand::thread_rng().gen_range(0.._useragent_list.len());
...
pub async fn get_user_agent(fetch_new_user_agents: bool, mut force_update: bool) -> String {
    ...
    if force_update {
        let agents = fetch_useragents_from_internet().await;
        match agents {
            Ok(agent) => {
                if agent.len() != 0 {
                    let _ = write_to_useragents_file(agent, force_update);
                } else {
                    warn!("recieved 0 user agents, will used pre-existing one");
                }
            }
            Err(e) => {
                warn!("Could not fetch agents, will use pre-existing one");
            }
        }
    }

    let agents_list = read_useragents_from_file().unwrap();
    if agents_list.len() != 0 {
        let num = rand::thread_rng().gen_range(0..agents_list.len());
        return agents_list[num].to_string().clone();
    }
...
pub const _useragent_list : [&str;345]= [
